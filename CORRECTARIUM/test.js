import { GetPrice,
    ...
    describe.each([
        ...
        ])('.GetWorkingTime($language, $symbolsCount, $extension)', ({language,symbolsCount, extension, expected}) => {
        test(`returns ${expected}`, () => {
            expect(GetWorkingTime(language,symbolsCount, extension)).toBe(expected);
        })})

        describe.each([
            {workingTime: 1, currentDate: new Date('Decembry 20, 2022 11:00:00'), expected: new Date('Decembry 20, 2022 12:00:00')},
            {workingTime: 8, currentDate: new Date('Decembry 20, 2022 11:00:00'), expected: new Date('Decembry 20, 2022 19:00:00')},
            {workingTime: 16, currentDate: new Date('Decembry 20, 2022 11:00:00'), expected: new Date('Decembry 21, 2022 18:00:00')},
            {workingTime: 100, currentDate: new Date('Decembry 24, 2022 11:00:00'), expected: new Date('January 10, 2023 11:00:00')},
            {workingTime: 9, currentDate: new Date('Decembry 23, 2022 19:00:00'), expected: new Date('Decembry 26, 2022 19:00:00')}
            ])('.GetDeadline($workingTime, $currentDate)', ({workingTime, currentDate, expected}) => {
            test(`returns ${expected}`, () => {
