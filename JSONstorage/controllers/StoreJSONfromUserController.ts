...
function ErrorHandlerIfNotJSON(err: any, req: Request, res: Response, next:any){
...
function AdditionlJSONChecks(DataFromUser: any):string {
    if (Array.isArray(DataFromUser)){
        ...
        for (let i = 0; i < DataFromUser.length; i++){
            if (JSON.stringify(DataFromUser[i])[0] != "{") return `Check all given JSONS. There is not JSON at ${i+1} position`
        } 
    }    
...
const StoreUsersJSON = async (req: Request, res: Response) => {
    ...
    if (additionalJSONCheck === 'JSON is valid'){ 
        await JSONcollection.insertOne({'route': route, 
