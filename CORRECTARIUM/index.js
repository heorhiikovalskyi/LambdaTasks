...
function AddWorkingTime(date, workingTime){
    ...
    if (additionalworkingHours <= hourstilldayEnd){
        ...
        date.setDate(date.getDate() + 1)
        date.setHours(startHour, 0, 0)
        date.setTime(date.getTime() + 3600000 * (additionalworkingHours -  hourstilldayEnd))
    }
}
...
function Shift(date, flag, startDay, workingTime){
    ...
    if (flag == 0 && (date.getDay() == 6 || date.getDay() == 0 || date.getDay() < startDay)){
        date.setDate(date.getDate() + 2)
    }
}
...
function GetDeadline(workingTime, currentDate){
    ...
    if (deadline.getHours() == startHour && deadline.getMinutes() == 0) { //if deadline goes to the morning of next day
