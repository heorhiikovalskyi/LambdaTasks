...
bot.onText(/^Privat/i, async (msg) => {
  const date = new Date();
  const URL = `https://api.privatbank.ua/p24api/exchange_rates?json&date=${date.getDate()}.${
    date.getMonth() + 1
  }.${date.getFullYear()}`;
  let currencyData;
  await axios.get(URL).then(
 ...
 EUR
 ${currencyData[0].purchaseRate.toFixed(3)}/${currencyData[0].saleRate.toFixed(
        3
      )}
 USD
 ${currencyData[1].purchaseRate.toFixed(3)}/${currencyData[1].saleRate.toFixed(
        3
      )}`;
